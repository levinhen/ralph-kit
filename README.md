...
4. **Dependency audit** (optional) — the splitting agent does not get to sign off on its own graph, so a *separate* agent can be given only the source PRD and the finished `prd.json` — never the splitter's reasoning — to re-derive every `dependsOn` edge and re-check that the `Covers:` clauses tile `userNeed`. Its findings are applied (or rejected in writing) and the resolved graph is recorded in `deps-audit.json`. The point is the missing *verification* edge — a story scheduled before the thing that would let anyone observe it is done, which no amount of implementation effort inside that story can fix. It costs one extra agent call and nothing enforces it: run it when the graph is worth a second opinion, skip it on a short or linear backlog.
...
├── deps-audit.json              # optional: a second agent's independent read of the dependency graph
...
`ralph/scripts/lint-prd.sh --run <run_id>` validates the result: dangling, forward, self, or cyclic `dependsOn` edges and `dependsOnRuns` entries naming runs that don't exist are all rejected. If the run has a `deps-audit.json`, it is compared against `prd.json` **edge for edge** — so editing a `dependsOn` after the audit ran invalidates the audit instead of quietly outliving it. That comparison, and a missing audit file, print `WARN:` and leave the exit code alone: the audit is advice, not a gate. `ralph.sh` runs the same lint at startup and refuses to start on a failing PRD. (`RALPH_REQUIRE_DEPS_AUDIT=1` promotes the audit warnings back to errors; `RALPH_SKIP_DEPS_AUDIT=1` silences them.)
...
4. The agent, per its playbook: implements **exactly that one story** (only the slice named in `Covers:`), runs the quality checks covering what it changed (a typecheck plus the tests for that slice — not the whole suite on every round), flips the story file to `passes: true` and writes `notes`, appends a structured progress record via `append-progress-json.sh` (one JSON line into `progress/<story_id>.jsonl`, plus optional `--shared-memory` items for reusable patterns), and commits everything as `feat: [US-xxx] - [Title]`.
...
| `RALPH_SKIP_DEPS_AUDIT` | `0` | `1` lints a run-scoped PRD without mentioning `deps-audit.json` at all |
